  ...
  return useBot(async (_,channel) => {
    for ( ;; ) {
      ...
      messages.forEach(message => {
        message.content.match(/https:\/\/(www\.)?youtu(\.be|be\.com\/watch\?).*/g)
          ...
          .map(async id => {
            ...
            if ( isTitleMissing && YOUTUBE_API_KEY ) {
              const res =await fetch(`https://www.googleapis.com/youtube/v3/videos?id=${id}&part=snippet&fields=items(snippet(title))&key=${YOUTUBE_API_KEY}`)
