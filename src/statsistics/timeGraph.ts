...
export async function createTimeGraph( input:SongsByUserAndTime[] ) {
  ...
  const x =scaleUtc().domain([ minTime, maxTime ]).range([margin.left,width -margin.right])
  const y =scaleLinear().domain([ 0, maxValue ]).range([height -margin.bottom,margin.top])
  ...
  style.setTextContent(input.map(({ user },i) => `.u-${user.id}{animation:h ${input.length *4}s linear ${(i -1) *4}s infinite;opacity:0}`).join("") +`@keyframes h{${
    [
      [0,0],
  ...
  // Graph + images
  const promiseArary =input.map(({ list, user }) => new Promise<void>(resolve => {
    if ( !user ) return

    const group =createNode("g")
    const imageSize =52
    avatarStore.get( user.id, imageSize, imageSize )
      .then(image => {
        group.appendChild(
          createNode("image")
  ...
  await Promise.all(promiseArary)

  fs.writeFile("download/pmstats-timegraph-animated.svg",svg.render(),err => err && console.log(err))
  svgArray.forEach(({ content, fileName }) => fs.writeFile("download/pmstats-timegraph-user-" +fileName,content,err => err && console.log(err)))
}
